import { emitFurnishing, FURNISHING_DEFAULTS } from 'shared-lib/castle_furnishings';

// Thin wrapper for furnishing kind 'barrel'. The recipe schema, local frame and
// geometry live in shared-lib/castle_furnishings.js; place it through
// furnishingPlacement() so lights/clearance share this root's transform.
class CastleBarrel extends Part {
  static lodBudgets = [1];
  static noImpostor = true;
  static params = { surfaceMembers: 0, ...FURNISHING_DEFAULTS.barrel };
  build(p) { emitFurnishing(this, 'barrel', p); }
}
