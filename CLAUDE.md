...
6. **MatterEngine3** - Kernel library (`libmatter_engine3.a`) for the procedural engine
   ...
   - Build: `make -C MatterEngine3` → `build/libmatter_engine3.a` + embedded shader/SPIR-V headers.
     The same Makefile also has a `viewer-lib` target producing
     `build/libmatter_engine3_viewer.a` — the SAME source list (`ME3_CPP` plus the
     `vk_*`/`vt_*` sources) compiled with the editor's flag set. That, not
     `libmatter_engine3.a`, is what `editor.exe` links
...
7. **MatterEditor** - Interactive editor application linking the kernel library
   ...
   - Build: `.DEFAULT_GOAL` is `windows`, so `make -C MatterEditor` →
     `build/windows/editor.exe`; `make -C MatterEditor linux` →
     `build/linux/editor`. Always launched from the MatterEditor/ working directory.
     **An engine-source change is only in the editor after `make -C MatterEditor`** —
     `make -C MatterEngine3` alone rebuilds the headless archive, not the viewer one
