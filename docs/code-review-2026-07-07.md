...
`OpenParticleSurfaceLib/src/surface.c` (~:651): same T4 unsigned-short index truncation pattern present but not scoped in the fix tasks; ticket separately if OPSL evolves beyond demo status.
