...
| Verb | Grammar | Effect |
|---|---|---|
| `agent` | `agent <v1 request JSON>` | Versioned, bounded request/result envelope. Dispatches typed agent operations through the existing ticketed `CommandRegistry`; writes one terminal JSON record to `MATTER_AGENT_RESULT_FILE`. Commands include discovery (`agent.commands` / `agent.help` / `agent.schema`), typed scene reads (`scene.list_objects` / `scene.get_object`), shared editor selection (`selection.replace` / `add` / `remove` / `toggle` / `clear` / `list`), and the interactive-picker-backed `viewport.pick` / `viewport.pick_select`. See `agent-protocol.md`. |
