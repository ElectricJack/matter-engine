...
|---|---|---|
| Bake `Event` queue (`events.h`, `matter_engine.cpp:342,676,4638`) | Notification (queued) | Typed events `bake.started/part_done/finished/error/aborted`, `stream.refine_tile` on the session hub. New subscribers use `lane::app`; the legacy API has a dedicated `lane::legacy_poll` subscription set. `WorldSession::poll_event` pumps at most one legacy-lane envelope, converts it to the old `Event`, and returns it, so E3 needs no frame-loop pump and preserves one-event-per-call FIFO behavior. Drop counters replace the silent cap. `bake.aborted` and the `bake_generation` field on every bake/stream event (smart-dune.7) are additive on top of that baseline: `bake.aborted` is the terminal event for a run that gave up (exactly one of `bake.finished`/`bake.aborted` follows every `bake.started`), emitted from the `BakeRunScope` guard around the worker's command switch rather than from each exit path; `bake_generation` is stamped by `Impl::emit_bake()` so an event names the run that emitted it, not whatever is current when a consumer drains. |
