...
//   - Failure is not transactional: on a limit breach `out` keeps whatever was
//     already emitted. Only the OOM path clears it. Treat a false return as
//     "discard `out`".
...
// emission, leaving `out` holding the instances produced so far. Both caps are
// checked against the state at the moment of the check: depth on entry,
// out.size() just before an emit.
// Depth-first body of flatten(). `world` is the composed transform of `hash`'s
// placement; a part missing from the graph, or present with an empty child
// vector, is a LEAF and emits one FlatInstance. Interior parts emit nothing of
// their own — they only compose transforms down to their children, so a part
