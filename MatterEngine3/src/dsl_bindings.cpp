...
static JSValue j_clipMarker(JSContext* c, JSValueConst, int n, JSValueConst* a){int argc=anim_user_argc(n);std::string s;float t=0;if(argc<2||!rig_finite_number(c,argc,a,0,t)||!anim_string(c,a[1],s))state_of(c)->set_rig_error("marker requires finite time and name");else state_of(c)->clip_marker(t,s);return JS_UNDEFINED;}
static JSValue j_clipKey(JSContext* c, JSValueConst, int n, JSValueConst* a){int argc=anim_user_argc(n);std::string s;float t=0;bool ok=argc>=3&&anim_string(c,a[0],s)&&rig_finite_number(c,argc,a,1,t);bool tr=false;matter::AnimationTransform v=socket_transform(c,a[2],&tr);if(!ok||!tr)state_of(c)->set_rig_error("key requires joint, finite time, and transform");else state_of(c)->clip_key(s,t,v);return JS_UNDEFINED;}
...
static JSValue j_motionInput(JSContext* c, JSValueConst, int n, JSValueConst* a){rig_source(c,n,a,"input");int argc=anim_user_argc(n);std::string name;if(argc<2||!anim_string(c,a[0],name)||!JS_IsObject(a[1])){state_of(c)->set_rig_error("input requires name and options");return JS_UNDEFINED;} matter::animation::InputSchema in;in.name=name;in.source={"<part>",0,0,"input"};JSValue v=JS_GetPropertyStr(c,a[1],"type");in.type=anim_type(c,v);JS_FreeValue(c,v);v=JS_GetPropertyStr(c,a[1],"cadence");if(JS_IsString(v))in.cadence=anim_cadence(c,v);JS_FreeValue(c,v);v=JS_GetPropertyStr(c,a[1],"default");if(!JS_IsUndefined(v))in.default_value=anim_value(c,v,in.type);JS_FreeValue(c,v);state_of(c)->motion_input(in);return JS_UNDEFINED;}
static JSValue j_motionTarget(JSContext* c, JSValueConst, int n, JSValueConst* a){rig_source(c,n,a,"target");int argc=anim_user_argc(n);std::string name;if(argc<2||!anim_string(c,a[0],name)||!JS_IsObject(a[1])){state_of(c)->set_rig_error("target requires name and options");return JS_UNDEFINED;} matter::animation::TargetSchema t;t.name=name;t.source={"<part>",0,0,"target"};auto getstr=[&](const char*k,std::string&o){JSValue v=JS_GetPropertyStr(c,a[1],k);bool ok=anim_string(c,v,o);JS_FreeValue(c,v);return ok;};if(!getstr("start",t.start_joint)||!getstr("end",t.end_joint)){state_of(c)->set_rig_error("target requires start and end joints");return JS_UNDEFINED;}JSValue v=JS_GetPropertyStr(c,a[1],"cadence");if(JS_IsString(v))t.cadence=anim_cadence(c,v);JS_FreeValue(c,v);v=JS_GetPropertyStr(c,a[1],"driver");if(JS_IsString(v)){t.driver=arg_string(c,v)=="controller"?matter::animation::TargetDriverKind::Controller:matter::animation::TargetDriverKind::External;}else if(JS_IsObject(v)){JSValue q=JS_GetPropertyStr(c,v,"controller");t.driver=matter::animation::TargetDriverKind::Controller;t.controller=arg_string(c,q);JS_FreeValue(c,q);}JS_FreeValue(c,v);v=JS_GetPropertyStr(c,a[1],"pole");if(!JS_IsUndefined(v)&&!JS_IsNull(v)){t.has_pole=array_floats(c,v,3,&t.pole.x);}JS_FreeValue(c,v);double d=0;auto num=[&](const char*k,float&o){JSValue q=JS_GetPropertyStr(c,a[1],k);if(JS_IsNumber(q)){JS_ToFloat64(c,&d,q);o=(float)d;}JS_FreeValue(c,q);};num("soften",t.soften);num("twist",t.twist);num("positionHalfLife",t.position_half_life);num("rotationHalfLife",t.rotation_half_life);num("weightHalfLife",t.weight_half_life);v=JS_GetPropertyStr(c,a[1],"enabled");if(!JS_IsUndefined(v))t.enabled=JS_ToBool(c,v)>0;JS_FreeValue(c,v);state_of(c)->motion_target(t);return JS_UNDEFINED;}
static JSValue j_motionController(JSContext* c, JSValueConst, int n, JSValueConst* a){rig_source(c,n,a,"controller");int argc=anim_user_argc(n);std::string name,type="native";if(argc<1||!anim_string(c,a[0],name)){state_of(c)->set_rig_error("controller requires a name");return JS_UNDEFINED;}if(argc>1&&JS_IsString(a[1]))type=arg_string(c,a[1]);matter::animation::ControllerDef d;d.name=name;d.type=type;d.source={"<part>",0,0,"controller"};if(argc>2&&JS_IsObject(a[2])){JSValue v=JS_GetPropertyStr(c,a[2],"cadence");if(JS_IsString(v))d.cadence=anim_cadence(c,v);JS_FreeValue(c,v);}state_of(c)->motion_controller(d);return JS_UNDEFINED;}
static JSValue j_motionNode(JSContext* c, JSValueConst, int n, JSValueConst* a, matter::animation::GraphNodeKind kind){int argc=anim_user_argc(n);std::string name;if(argc<1||!anim_string(c,a[0],name)){state_of(c)->set_rig_error("graph node requires a name");return JS_UNDEFINED;}matter::animation::GraphNode g;g.name=name;g.kind=kind;g.source={"<part>",0,0,"graph"};for(int i=1;i<argc;++i){std::string dep;if(anim_string(c,a[i],dep))g.dependencies.push_back(dep);}g.is_output=kind==matter::animation::GraphNodeKind::Output;state_of(c)->motion_node(g);return JS_UNDEFINED;}
static JSValue j_clipNode(JSContext* c,JSValueConst,int n,JSValueConst*a){int argc=anim_user_argc(n);if(argc<2){state_of(c)->set_rig_error("clipNode requires name and clip");return JS_UNDEFINED;}matter::animation::GraphNode g;g.name=arg_string(c,a[0]);g.clip=arg_string(c,a[1]);g.kind=matter::animation::GraphNodeKind::Clip;g.source={"<part>",0,0,"graph"};state_of(c)->motion_node(g);return JS_UNDEFINED;} static JSValue j_blendNode(JSContext*c,JSValueConst,int n,JSValueConst*a){int argc=anim_user_argc(n);if(argc<3||!JS_IsArray(a[2])){state_of(c)->set_rig_error("blend1D requires name, input, and samples");return JS_UNDEFINED;}matter::animation::GraphNode g;g.name=arg_string(c,a[0]);g.input=arg_string(c,a[1]);g.kind=matter::animation::GraphNodeKind::Blend1D;g.source={"<part>",0,0,"graph"};JSValue lv=JS_GetPropertyStr(c,a[2],"length");uint32_t len=0;JS_ToUint32(c,&len,lv);JS_FreeValue(c,lv);for(uint32_t i=0;i<len;++i){JSValue pair=JS_GetPropertyUint32(c,a[2],i);if(JS_IsArray(pair)){JSValue tv=JS_GetPropertyUint32(c,pair,0);if(JS_IsNumber(tv))g.thresholds.push_back((float)argd(c,tv));JS_FreeValue(c,tv);JSValue nv=JS_GetPropertyUint32(c,pair,1);std::string dep;if(anim_string(c,nv,dep))g.dependencies.push_back(dep);JS_FreeValue(c,nv);}JS_FreeValue(c,pair);}state_of(c)->motion_node(g);return JS_UNDEFINED;} static JSValue j_additiveNode(JSContext*c,JSValueConst,int n,JSValueConst*a){return j_motionNode(c,JS_UNDEFINED,n,a,matter::animation::GraphNodeKind::Additive);} static JSValue j_nativeNode(JSContext*c,JSValueConst,int n,JSValueConst*a){int argc=anim_user_argc(n);if(argc<2){state_of(c)->set_rig_error("nativeController requires name and controller");return JS_UNDEFINED;}matter::animation::GraphNode g;g.name=arg_string(c,a[0]);g.controller=arg_string(c,a[1]);g.kind=matter::animation::GraphNodeKind::NativeController;g.source={"<part>",0,0,"graph"};if(argc>2){std::string dep;if(anim_string(c,a[2],dep))g.dependencies.push_back(dep);}state_of(c)->motion_node(g);return JS_UNDEFINED;} static JSValue j_outputNode(JSContext*c,JSValueConst,int n,JSValueConst*a){return j_motionNode(c,JS_UNDEFINED,n,a,matter::animation::GraphNodeKind::Output);}
