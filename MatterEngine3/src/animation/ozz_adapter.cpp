// MatterEngine3/src/animation/ozz_adapter.cpp
//
// Runtime half of the ozz-animation adapter declared in `ozz_adapter.h`.
// Together with the bake-only `ozz_bake.cpp` these are the only translation
// units that speak ozz's SoA math and job API; everything above them -- the
// evaluator, the target solver, the runtime asset -- sees only Matter IR
// types (`AnimationTransform`, `Mat4f`, `JointIndex`) and byte buffers.
//
// What lives here
//   - `BufferStream`: an in-memory `ozz::io::Stream`, so archives are read
//     from and written to a `std::vector<uint8_t>` with no filesystem.
//   - The Matter <-> ozz conversions: AoS/SoA transform packing and the
//     row-major/column-major matrix transpose.
//   - The `MOS1` / `MOA1` archive containers and their validation.
//   - The four runtime operations: `sample`, `blend`, `local_to_model` and
//     `solve_two_bone`.
//
// Conventions and gotchas
//   - Every entry point returns bool and reports failure by returning false;
//     nothing here throws and nothing logs.  Only the two `deserialize_*`
//     functions also append `ozz-archive` diagnostics.
//   - `Mat4f` stores rows contiguously (`m[row * 4 + column]`) while ozz's
//     `Float4x4` is four column vectors.  `to_ozz_matrix` /
//     `from_ozz_matrix` are the ONLY place that transposition happens -- do
//     not hand a `Mat4f` to an ozz job directly.
//   - Joint ordering is Matter's canonical depth-first order (see
//     `animation_validate.cpp`).  The parent/subtree metadata that rides
//     alongside the ozz archive is cross-checked against ozz's own joint
//     parents at load, which is what ties the two orderings together.
//   - ozz jobs work four joints at a time, so SoA buffers are rounded up to a
//     multiple of four; the padding lanes hold identity transforms and are
//     dropped again by `from_soa`.
//   - The only allocation that persists between calls is inside
//     `OzzSampleContext`; every other function allocates temporary SoA
//     buffers per call.  None of these functions synchronize -- one context /
//     one skeleton must not be driven by two concurrent calls.
...
// In-memory `ozz::io::Stream` over a byte vector, so ozz archives can be read
// out of and written into a blob with no file involved.
//
// Gotcha: a short `Read` does NOT fail the way a file stream would.  ozz's
// archive reader assumes its reads succeed, so a truncated read zero-fills the
// remainder, advances the cursor past the end and still returns the requested
// size -- it only latches `failed_`.  Callers must therefore check `failed()`,
// and the deserializers below additionally require `Tell()` to land exactly on
// the buffer size, before trusting anything they decoded.
...
// Matter keeps one transform per joint (AoS); ozz's jobs consume four joints
// per SIMD group (SoA).  `to_soa` rounds the group count up and pads the
// unused lanes with an identity transform; `from_soa` writes only the first
// `count` joints back, dropping that padding.  Neither validates -- a caller
// that passes a mismatched `count` silently reads padding.
...
// The one transposition point between the two matrix layouts: `Mat4f` stores
// rows contiguously (`m[row * 4 + column]`), ozz's `Float4x4` stores four
// column vectors.  Getting this backwards produces a transposed-but-plausible
// pose, so route every conversion through these two.
...
// Rejects skeleton metadata that a canonical Matter rig could not have
// produced -- this is the guard against a corrupt or hand-edited archive, not
// just a bounds check.  Requirements: joint 0 is the root, every other joint
// has a parent at a strictly lower index, `subtree.begin` equals the joint's
// own index, and the subtree ends recomputed bottom-up match the stored ones
// EXACTLY (not merely "contain them").
...
// Writes the `MOS1` container: the four magic bytes, a little-endian u16 joint
// count, a (parent, subtree.begin, subtree.end) u16 triple per joint, and then
// the ozz skeleton archive appended through the same stream.  `bytes` is
// replaced wholesale on success and left untouched when the skeleton was never
// built (checked before anything is written).
...
// Writes the `MOA1` container: magic bytes, a little-endian u16 track count,
// then the ozz animation archive.  The track count is redundant with the
// archive and exists so the loader can cross-check it.
...
// Inverse of `serialize_skeleton`, and the trust boundary for skeleton blobs.
// It checks, in order: the magic, the joint budget and header length, the
// metadata shape (`validate_metadata`), the ozz archive tag, that the stream
// neither short-read nor left trailing bytes, that ozz's decoded joint count
// matches the header, and that ozz's own joint parents agree with the Matter
// metadata joint for joint.  Any mismatch adds an `ozz-archive` diagnostic and
// returns false with `skeleton` untouched.  On success it also caches the
// rest-pose locals decoded from the archive, which is what `rest_local()`
// serves.
...
// Inverse of `serialize_animation`; same trust rules as the skeleton loader --
// magic, archive tag, no short read, no trailing bytes, and the decoded track
// count must equal the header's.  Adds an `ozz-archive` diagnostic and returns
...
// Samples `animation` into `locals`, one local transform per animation track.
// `ratio` is a NORMALIZED phase in [0,1], not seconds, and is clamped rather
// than wrapped -- looping is the caller's job.
//
// `context` is grown lazily: the first call whose animation has more tracks
// than the context was built for reallocates it, and it never shrinks.  Reuse
// one context per concurrently sampled clip instance (sharing one across two
// clips defeats ozz's interpolation cache); `mutable_bytes_for_tracks()`
// reports what that costs the animation budget.  Returns false for an unbuilt
// animation, a non-finite ratio, or a rejected ozz job.
...
// Weighted blend of `layers`, then `additive_layers` applied on top as
// bind-relative deltas, into `locals`.  The skeleton's rest pose is handed to
// ozz as the fallback for joints the accumulated layer weight does not cover.
//
// Layers whose `locals` pointer is null are skipped; every non-null one must
// have exactly `skeleton.joint_count()` entries, and an additive layer alone
// (no normal layer to establish the count) is rejected.  Weights are passed
// through unnormalized.  Allocates a SoA copy of every layer plus the rest
// pose on each call.
...
// Converts parent-relative `locals` into model-space matrices.
//
// `affected` defaults to the whole-skeleton sentinel
// ({kInvalidJoint, kInvalidJoint}).  Any other range must be EXACTLY
// `skeleton.subtree(affected.begin)` -- an arbitrary sub-range is rejected --
// and a partial range that does not start at joint 0 additionally requires
// `models` to already be sized to the joint count, because the entries outside
// the range are read back in as the parent matrices the subtree hangs off.
// Only the joints inside the range are written.  Returns false on any shape
// mismatch rather than resizing to fit.
...
// Runs ozz's two-bone IK for `solve`, multiplies the resulting start/mid
// corrections into `locals` (renormalizing each rotation), and re-derives the
// affected subtree's model matrices into `updated_models`.
//
