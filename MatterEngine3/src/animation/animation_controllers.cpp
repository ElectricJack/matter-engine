...
Float3 transform_point(const NativeControllerContext& c,Float3 v){if(!c.has_entity_world)return v;const Mat4f&m=c.entity_world;return {m.m[0]*v.x+m.m[1]*v.y+m.m[2]*v.z+m.m[3],m.m[4]*v.x+m.m[5]*v.y+m.m[6]*v.z+m.m[7],m.m[8]*v.x+m.m[9]*v.y+m.m[10]*v.z+m.m[11]};}
...
private: struct State{double time=0;Foot feet[2]{};};
 bool foot(NativeControllerContext&c,State& next,std::vector<ControllerTargetWrite>& writes,int index,uint16_t target,Float3 predicted){ const float phase=std::fmod(static_cast<float>(next.time/p_.stride_seconds)+(index?0.5f:0.f),1.f); const bool stance=phase<0.5f; Foot&f=next.feet[index];WorldRayHit hit{};const Float3 predicted_world=transform_point(c,predicted); const Float3 from{predicted_world.x,predicted_world.y+p_.step_height,predicted_world.z};const bool got=c.world_queries->ray_cast(from,{0,-1,0},p_.ray_distance,0,hit);const bool walkable=got&&finite3(hit.position)&&finite3(hit.normal)&&hit.normal.y>=p_.min_ground_normal_y&&std::fabs(hit.position.y-predicted_world.y)<=p_.step_height;
