...
namespace {
bool finite(float v){return std::isfinite(v);} bool finite3(Float3 v){return finite(v.x)&&finite(v.y)&&finite(v.z);} bool valid(const GaitControllerParameters&p){return p.left_target!=UINT16_MAX&&p.right_target!=UINT16_MAX&&p.left_target!=p.right_target&&finite3(p.left_predicted)&&finite3(p.right_predicted)&&finite(p.stride_seconds)&&p.stride_seconds>0&&finite(p.swing_height)&&p.swing_height>=0&&finite(p.ray_distance)&&p.ray_distance>0&&finite(p.step_height)&&p.step_height>=0&&finite(p.min_ground_normal_y)&&p.min_ground_normal_y>=-1&&p.min_ground_normal_y<=1;}
Quaternion normalize(Quaternion q){const float n=std::sqrt(q.x*q.x+q.y*q.y+q.z*q.z+q.w*q.w);return n>1e-6f?Quaternion{q.x/n,q.y/n,q.z/n,q.w/n}:Quaternion{};}
Float3 rotate(Quaternion q,Float3 v){q=normalize(q);const Float3 u{q.x,q.y,q.z};const Float3 uv{u.y*v.z-u.z*v.y,u.z*v.x-u.x*v.z,u.x*v.y-u.y*v.x};const Float3 uuv{u.y*uv.z-u.z*uv.y,u.z*uv.x-u.x*uv.z,u.x*uv.y-u.y*uv.x};return {v.x+2.f*(q.w*uv.x+uuv.x),v.y+2.f*(q.w*uv.y+uuv.y),v.z+2.f*(q.w*uv.z+uuv.z)};}
...
public: explicit GaitController(GaitControllerParameters p):p_(p){}
 ...
 bool fixed_update(NativeControllerContext& c) override { if(!c.world_queries||!std::isfinite(c.fixed_delta_seconds)||c.fixed_delta_seconds<0)return false; double speed=1.0; if(!c.inputs.empty()){if(c.inputs[0].type!=AnimationValueType::Number||!std::isfinite(c.inputs[0].number)||c.inputs[0].number<0)return false;speed=c.inputs[0].number;} State next=state_; std::vector<ControllerTargetWrite> writes; if(!foot(c,next,writes,0,p_.left_target,p_.left_predicted)||!foot(c,next,writes,1,p_.right_target,p_.right_predicted)) return false; next.time+=c.fixed_delta_seconds*speed; if(!std::isfinite(next.time)) return false; c.writes.insert(c.writes.end(),writes.begin(),writes.end()); state_=next; return true; }
...
private: struct State{double time=0;Foot feet[2]{};};
 bool foot(NativeControllerContext&c,State& next,std::vector<ControllerTargetWrite>& writes,int index,uint16_t target,Float3 predicted){ const float phase=std::fmod(static_cast<float>(next.time/p_.stride_seconds)+(index?0.5f:0.f),1.f); const bool stance=phase<0.5f; Foot&f=next.feet[index];WorldRayHit hit{};const Float3 predicted_world=rotate(c.entity_world_rotation,predicted); const Float3 from{c.entity_world_origin.x+predicted_world.x,c.entity_world_origin.y+predicted_world.y+p_.step_height,c.entity_world_origin.z+predicted_world.z};const bool got=c.world_queries->ray_cast(from,{0,-1,0},p_.ray_distance,0,hit);const bool walkable=got&&finite3(hit.position)&&finite3(hit.normal)&&hit.normal.y>=p_.min_ground_normal_y;
  ...
  AnimationTransform result{}; result.translation=f.planted?f.position:Float3{c.entity_world_origin.x+predicted_world.x,c.entity_world_origin.y+predicted_world.y,c.entity_world_origin.z+predicted_world.z}; if(!stance){const float x=(phase-.5f)*2.f;result.translation.y+=p_.swing_height*(3*x*x-2*x*x*x);} if(!finite3(result.translation)) return false; writes.push_back({target,result,1});return true; }
