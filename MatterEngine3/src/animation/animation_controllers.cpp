...
private: struct State{double time=0;Foot feet[2]{};};
 bool foot(NativeControllerContext&c,State& next,std::vector<ControllerTargetWrite>& writes,int index,uint16_t target,Float3 predicted){ const float phase=std::fmod(static_cast<float>(next.time/p_.stride_seconds)+(index?0.5f:0.f),1.f); const bool stance=phase<0.5f; Foot&f=next.feet[index];WorldRayHit hit{};const Float3 from{predicted.x,predicted.y+p_.step_height,predicted.z};const bool got=c.world_queries->ray_cast(from,{0,-1,0},p_.ray_distance,0,hit);const bool walkable=got&&finite3(hit.position)&&finite3(hit.normal)&&hit.normal.y>=p_.min_ground_normal_y;
  ...
  AnimationTransform result{}; result.translation=f.planted?f.position:predicted; if(!stance){const float x=(phase-.5f)*2.f;result.translation.y+=p_.swing_height*(3*x*x-2*x*x*x);} if(!finite3(result.translation)) return false; writes.push_back({target,result,1});return true; }
