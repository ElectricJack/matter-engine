...
bool save_anim_candidate(const AnimAsset& a, const std::filesystem::path& path, Diagnostics& d) {
    ...
    u64(out,a.resolved_hash); u64(out,a.nonce.high); u64(out,a.nonce.low); u32(out,a.target_abi_tag); u32(out,a.ozz_tag_hash); u32(out,uint32_t(a.sections.size())); u64(out,kHeaderBytes); u64(out,fnv(payload.data(),payload.size()));
    ...
    std::error_code ec; std::filesystem::create_directories(path.parent_path(),ec); FILE* f=std::fopen(path.string().c_str(),"wb"); if(!f){fail(d,"anim.write");return false;} bool ok=std::fwrite(out.data(),1,out.size(),f)==out.size() && std::fflush(f)==0; std::fclose(f); if(!ok) fail(d,"anim.write"); return ok;
}
...
bool load_anim(const std::filesystem::path& path, AnimAsset& a, Diagnostics& d) {
    ...
    if (fnv(b.data()+table_end,b.size()-size_t(table_end)) != checksum) { fail(d,"anim.checksum"); return false; }
    for (const auto& e : es) {
