...
bool rollback(const std::filesystem::path& path,const BuildNonce& n, bool existed) { std::error_code ec; if(existed)return part_asset::replace_file_atomic(backup(path,n).string(),path.string()); const bool present=std::filesystem::exists(path,ec); if(ec)return false; return !present || (std::filesystem::remove(path,ec)&&!ec&&sync_parent(path)); }
...
bool publish_animation_bundle(const BundleCandidates& c,const BundleIdentity& i,Diagnostics& d){
    ...
    if(!part_asset::replace_file_atomic(c.part_candidate.string(),part.string())){if(!cleanup())fail(d,"bundle.cleanup");fail(d,"bundle.publish");return false;}
    if(c.test_fail_before_replace==2){const bool restored=rollback(part,i.nonce,had_part);if(!restored)fail(d,"bundle.rollback");if(!cleanup())fail(d,"bundle.cleanup");fail(d,"bundle.injected");return false;}
    if(!part_asset::replace_file_atomic(c.anim_candidate.string(),anim.string())){const bool restored=rollback(part,i.nonce,had_part);if(!restored)fail(d,"bundle.rollback");if(!cleanup())fail(d,"bundle.cleanup");fail(d,"bundle.publish");return false;}
    if(c.test_fail_before_replace==3){const bool restored=rollback(anim,i.nonce,had_anim)&&rollback(part,i.nonce,had_part);if(!restored)fail(d,"bundle.rollback");if(!cleanup())fail(d,"bundle.cleanup");fail(d,"bundle.injected");return false;}
    if(!part_asset::replace_file_atomic(tmp(manifest,i.nonce).string(),manifest.string())){const bool restored=rollback(anim,i.nonce,had_anim)&&rollback(part,i.nonce,had_part);if(!restored)fail(d,"bundle.rollback");if(!cleanup())fail(d,"bundle.cleanup");fail(d,"bundle.publish");return false;}
    if(!cleanup()){fail(d,"bundle.cleanup");return false;}return true;
}
