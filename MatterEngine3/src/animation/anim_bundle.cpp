...
std::filesystem::path backup(const std::filesystem::path&p){return p.string()+".bundle.backup";}
bool backup_existing(const std::filesystem::path& path, bool& existed) { std::error_code ec; existed=std::filesystem::exists(path,ec); return !ec && (!existed || std::filesystem::copy_file(path,backup(path),std::filesystem::copy_options::overwrite_existing,ec)); }
void rollback(const std::filesystem::path& path, bool existed) { std::error_code ec; if(existed) part_asset::replace_file_atomic(backup(path).string(),path.string()); else std::filesystem::remove(path,ec); }
void discard_backup(const std::filesystem::path& path) { std::error_code ec; std::filesystem::remove(backup(path),ec); }
...
bool publish_animation_bundle(const BundleCandidates& c,const BundleIdentity& i,Diagnostics& d){
    ...
    if(!make_manifest(i,m)||!write(tmp(manifest),m)||!backup_existing(part,had_part)||!backup_existing(anim,had_anim)||!backup_existing(manifest,had_manifest)){fail(d,"bundle.publish");return false;}
    if(c.test_fail_before_replace==1){discard_backup(part);discard_backup(anim);discard_backup(manifest);fail(d,"bundle.injected");return false;}
    if(!part_asset::replace_file_atomic(c.part_candidate.string(),part.string())){discard_backup(part);discard_backup(anim);discard_backup(manifest);fail(d,"bundle.publish");return false;}
    if(c.test_fail_before_replace==2){rollback(part,had_part);discard_backup(anim);discard_backup(manifest);fail(d,"bundle.injected");return false;}
    if(!part_asset::replace_file_atomic(c.anim_candidate.string(),anim.string())){rollback(part,had_part);discard_backup(anim);discard_backup(manifest);fail(d,"bundle.publish");return false;}
    if(c.test_fail_before_replace==3){rollback(anim,had_anim);rollback(part,had_part);discard_backup(manifest);fail(d,"bundle.injected");return false;}
    if(!part_asset::replace_file_atomic(tmp(manifest).string(),manifest.string())){rollback(anim,had_anim);rollback(part,had_part);discard_backup(manifest);fail(d,"bundle.publish");return false;}
    discard_backup(part);discard_backup(anim);discard_backup(manifest);return true;
}
