...
bool finite_transform(const AnimationTransform& v) { return finite(v.translation.x)&&finite(v.translation.y)&&finite(v.translation.z)&&finite(v.rotation.x)&&finite(v.rotation.y)&&finite(v.rotation.z)&&finite(v.rotation.w)&&finite(v.scale.x)&&finite(v.scale.y)&&finite(v.scale.z); }
Float3 lerp(Float3 a, Float3 b, float t) { return {a.x+(b.x-a.x)*t,a.y+(b.y-a.y)*t,a.z+(b.z-a.z)*t}; }
Quaternion normalize(Quaternion q) { const float n=std::sqrt(q.x*q.x+q.y*q.y+q.z*q.z+q.w*q.w); return n>1e-7f?Quaternion{q.x/n,q.y/n,q.z/n,q.w/n}:Quaternion{}; }
...
bool smooth_animation_target(const CanonicalTarget& target, AnimationTargetState& state, double dt, EvaluationCadence cadence) {
    ...
    state.evaluated_weight=clamp01(state.evaluated_weight); return true;
}

bool resolve_two_bone_chain(const OzzSkeleton& skeleton, JointIndex start, JointIndex end, JointIndex& mid, JointRange& affected) {
    if(start==kInvalidJoint||end==kInvalidJoint||start>=skeleton.joint_count()||end>=skeleton.joint_count())return false;
    mid=skeleton.parent(end); if(mid==kInvalidJoint||skeleton.parent(mid)!=start)return false;
    affected=skeleton.subtree(start); return affected.begin!=kInvalidJoint&&affected.end>affected.begin;
}
...
bool solve_animation_target(const CanonicalTarget& t,const OzzSkeleton&s,const AnimationTargetState& state,std::vector<AnimationTransform>&locals,std::vector<Mat4f>&models) {
    ...
    const JointIndex parent=s.parent(t.chain[2]); if(parent==kInvalidJoint)return false;
    ...
    locals[t.chain[2]].rotation=slerp(locals[t.chain[2]].rotation,local_target,state.evaluated_weight);
