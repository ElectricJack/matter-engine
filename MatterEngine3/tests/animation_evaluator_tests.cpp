...
void test_blend_and_additive(){
 ...
 f.def.nodes[2].cadence=EvaluationCadence::Fixed;AnimationEvaluator invalid_cadence;CHECK(!invalid_cadence.evaluate({frame_request}),"frame controls cannot feed fixed graph nodes");f.def.inputs[0].cadence=EvaluationCadence::Fixed;
...
int main(){test_controls();test_cadence_aware_control_sampling();test_time_interpolation_and_previous();test_wrap_clamp_pause_and_disable();test_non_looping_clip_clamps();test_blend_and_additive();test_budget_order_and_reuse();test_snapshot_backing_and_priority_controller_budget();if(g_failures){std::printf("animation_evaluator_tests: %d failure(s)\n",g_failures);return 1;}std::puts("animation_evaluator_tests: all tests passed");}
