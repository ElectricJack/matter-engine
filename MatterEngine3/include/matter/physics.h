...
// Import this to bring physics into a world (`world.import<PhysicsModule>()`).
// The constructor creates the solver context, registers the component types,
// the reconcile observers and the four fixed-pipeline systems. Everything below
// is inert on a world that never imported it.
struct PhysicsModule {
    explicit PhysicsModule(flecs::world&);
};

// ---------------------------------------------------------------------------
// Session-level accessors and commands.
//
// The two accessors are safe on a world without the module: `physics_events`
// returns a shared empty snapshot and `physics_stats` a zeroed struct. The
// returned events reference is only valid until the next pull stage.
//
// The five `bool` commands are DEFERRED — they enqueue onto the context and are
// applied at the next push stage, not at the call. They return false (and do
// nothing) unless the entity is alive, carries a `RigidBody` of type Dynamic,
// has no `PhysicsError`, and its world has the module. So `false` means "this
// entity is not commandable", which is a normal outcome, not an exception.
//
// The two queries run immediately against the context's broadphase tree.
// `physics_ray_cast`'s third argument is the ray's full TRANSLATION (origin +
// translation is the far end), not a normalized direction — a unit vector casts
// exactly one metre. `category_mask` is tested against shapes'
