...
//     runs INSIDE a publish job on the thread that owns the render device
//     (`matter_async::assert_gl_thread` — the symbol keeps its pre-Vulkan
//     name) — so, unlike on_mesh_ready, this callback may already be on the
//     render thread. Treat both callbacks as "unknown thread, not
//     necessarily the render thread" and never assume otherwise: a caller
//     that invokes bake_lods() from a worker thread (as headless tests and
//     any future off-render-thread LOD bake path may do) gets the same
//     contract.
//   - The callee MUST NOT touch the render device, ImGui, or any
//     windowing/render state directly from inside either callback,
//     regardless of which thread it happens to land on. Copy whatever data
//     you need and marshal actual GPU/UI work through the session's existing
//     GpuJobQueue (pump_gpu_jobs) or event mechanism (poll_event) — never act
//     on render state synchronously inside the callback.
