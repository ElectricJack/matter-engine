...
Matrix4x4 matrix_inverse(const Matrix4x4* m) {
    // Simplified inverse for now - in production, use proper 4x4 inverse
    // For testing, return identity if it's identity matrix
    Matrix4x4 result;
    ...
    for (int i = 0; i < 16; i++) {
        if (fabsf(m->m[i] - identity.m[i]) > 0.001f) {
            is_identity = false;
            break;
        }
    }
    ...
    if (is_identity) {
        ...
        // For now, return identity - proper inverse calculation would go here
        result = identity;
    }
